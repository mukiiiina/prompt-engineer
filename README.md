...
- Prompts generated by this skill are based on visual analysis of websites, screenshots, or text descriptions. While we strive for accuracy, **we do not guarantee 100% pixel-perfect replication** of the original design.
- This skill and its outputs are intended for **learning, prototyping, and personal reference only**. Please respect the intellectual property rights of original websites. Do not use generated prompts or code for commercial infringement, unauthorized cloning, or any activity that violates the original site's terms of service.
- Demo assets (e.g., images in `assets/demo-*.html`) use placeholder photos from **Unsplash** or similar services. All photo copyrights belong to their respective authors. Replace them with your own assets before production use.
- The authors of this skill are **not affiliated with, endorsed by, or sponsored by** any of the websites referenced in examples (e.g., Studio DADO, Linear, Midlife Engineering). All trademarks and brand names belong to their respective owners.
- **Use at your own risk.** The authors assume no liability for any legal, technical, or business issues arising from the use of this skill or its outputs.

**中文：**

- 本 skill 生成的提示词基于对网站外观、截图或文字描述的分析。我们尽力保证准确性，但**不保证与原网站 100% 像素级一致**。
- 本 skill 及其输出仅供**学习、原型设计和个人参考**。请尊重原网站的知识产权，请勿将生成的提示词或代码用于商业侵权、未经授权的复刻，或任何违反原网站服务条款的活动。
- 演示资源（如 `assets/demo-*.html` 中的图片）使用来自 **Unsplash** 等服务的占位照片，版权归原作者所有。在正式投产前请替换为你自己的素材。
- 本 skill 作者**与示例中引用的任何网站（如 Studio DADO、Linear、Midlife Engineering 等）均无关联、未获其背书或赞助**。所有商标和品牌名称均归其 respective 所有者所有。
- **使用风险自负。** 作者不对因使用本 skill 或其输出而产生的任何法律、技术或商业问题承担责任。
